...
onBeforeMount(async () => {
	...
	api.getConfig().then((v) => { config.value = v })
	api.getWhitelist().then((v) => { whitelist.value = v })
	api.getBlacklist().then((v) => { blacklist.value = v })
})
...
<template>
	<main>
		<div>
			<button :disabled="pending" @click="pend() && refresh()">
				Refresh
			</button>
		</div>
		<div class="config">
			...
			<div v-if="config">
				<div>
					...
					<input
						...
						@click.prevent="(event) => pend() && config?.set('onlineMode', !config?.onlineMode).catch(alert).then(refresh)"
						/>
				</div>
				<div>
					...
					<input
						...
						@click.prevent="(event) => pend() && config?.set('enableWhitelist', !config?.enableWhitelist).catch(alert).then(refresh)"
						/>
				</div>
				<div>
					...
					<input
						...
						@click.prevent="(event) => pend() && config?.set('enableIPWhitelist', !config?.enableIPWhitelist).catch(alert).then(refresh)"
						/>
				</div>
			</div>
			<div v-else>
			...
			<div v-if="whitelist">
				...
				<ul>
					<li v-for="(p, i) in whitelist.players" :key="p.id">
						...
						<button :disabled="pending" @click="pend() && whitelist.removePlayer(i).catch(alert).then(refresh)">-</button>
					</li>
					<li>
			...
			<div v-if="blacklist">
				...
				<ul>
					<li v-for="(p, i) in blacklist.players" :key="p.id">
						{{p.name}}
						<span v-if="p.isOffline"><i>(offline player)</i></span>
						<button :disabled="pending" @click="pend() && blacklist.removePlayer(i).catch(alert).then(refresh)">-</button>
					</li>
					<li>
